defmodule Relacoes do
  ...
  def compose(a, b, ai, bi, acc) do
    # IO.inspect("_________________________________________________________________")
    # IO.inspect([a, b, ai, bi, acc])
    # IO.inspect(ai < Enum.count(a))
    if(ai < Enum.count(a) and Enum.count(a)>0 and Enum.count(b)>0) do
      aa = Enum.at(a, ai)
      bb = Enum.at(b, bi)
      # IO.inspect("----------------------------------------------------------------")
